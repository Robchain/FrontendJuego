...
const SeleccionQue = ({ QueSelecion, data,  window, ...props}) => {
  ...
  useEffect(() => {
    if (QueSelecion === 1) { setSeleccionPal(data[`Juego` + window.id].Oraciones[0].Que) }
    if (QueSelecion === 2) { setSeleccionPal(data[`Juego` + window.id].Oraciones[1].Que) }
    if (QueSelecion === 3) { setSeleccionPal(data[`Juego` + window.id].Oraciones[2].Que) }
  }, [QueSelecion])
...
const RespuestaImagen = ({ Queselec, data,  window, setMomento, momento }) => {
  ...
  useEffect(() => {
    if (data[`Juego` + window.id].Oraciones[0].Respuesta === "CORRECTO") {
      AdjectivoRespuesta = data[`Juego` + window.id].Oraciones[0].Que;
    } else if (data[`Juego` + window.id].Oraciones[1].Respuesta === "CORRECTO") {
      AdjectivoRespuesta = data[`Juego` + window.id].Oraciones[1].Que;
    } else if (data[`Juego` + window.id].Oraciones[2].Respuesta === "CORRECTO") {
    ...
    <div  className='contenido-una-oracion'>
      ...
      <div className='seccion-opciones-oracion'>
          ...
          <div style={{ pointerEvents: pointerEvent, opacity: opacity1 }} onClick={onhandleClickQuePrimero}>
            ...
            <img src={data[`Juego` + window.id].Oraciones[0].Que.value}  alt='opcion1' className='opcion-imagen-neta'/>
            </div>
          </div>
          <div style={{ pointerEvents: pointerEvent, opacity: opacity2 }} onClick={onhandleClickQueSegundo}>
            <div className='opcion-imagen-una'>
            <img src={data[`Juego` + window.id].Oraciones[1].Que.value}  alt='opcion2' className='opcion-imagen-neta' />
            </div>
          </div>
          <div style={{ pointerEvents: pointerEvent, opacity: opacity3 }} onClick={onhandleClickQueTercero}>
            <div className='opcion-imagen-una'>
            <img src={data[`Juego` + window.id].Oraciones[2].Que.value} alt='opcion3' className='opcion-imagen-neta' />
            </div>
          </div>
      ...
      <div className='respuesta-seccion'>
        <RespuestaImagen momento={momento} setMomento={setMomento} Queselec={Queselec} data={data}  window={window} />
        </div>
      </div>
    </div>
  )
